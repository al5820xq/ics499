...
if ($profile->isUser()) {
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($name_err) && empty($animal_err) && empty($color_err) && empty($chipid_err) && empty($filename_err)){
            if ($profile->registerPet($name, $animal, $color, $chipid, $filename)) {
                header("Location: welcome.php");
            } else {
                echo '<h1>Error registering pet</h1>';
            }
...
<div class="small-container2">
            <div class="col-1"> 
                <h1 class="title">Register Pet</h1>
                <p>Please fill in pet's information.</p><br>
            </div>    
